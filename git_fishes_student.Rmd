...
[My GitHub Repository](https://github.com/bryn33/biol3207_workshop6)
...
## Github is not a backup service. Github uses a shared responsibility model which involves the user having to secure their own data. Github is used primary for version tracking and collaboration and lacks several important backup capabilities.
...
# Echo = FALSE prevents code, but not the results from appearing in the finished file and the code does not run.
```
...
path <- "output/data/OA_activitydat_20190302_BIOL3207.cs"
...
```{r, rmMiss}
data = data %>% filter_at(vars(loc,species,treatment,animal_id,sl,size,activity,comment), all_vars(!(is.na(.))))
...
#data = select (data,-c(comment,loc))

# Check spelling in species and treatment but also generate a summary table

check_treatment = nrow(data[which(data$treatment != c("control") & data$treatment != c("CO2")), ])

check_species = nrow(data[which(data$species != c("acantho") & data$species != c("ambon") & data$species != c("chromis")& data$species != c("humbug")& data$species != c("lemon")& data$species != c("whitedams")), ])

sum_table = data %>% group_by(treatment,species) %>% 
    summarise(
        mean_sl = mean(sl),
        std_sl = sd(sl),
        mean_activity = mean(activity),
        std_activity = sd(activity),
        sample_size = n()
    )
...
## One side shows the additions to the document and the other side shows deletions.

## Green highlights mean changes in that file that were added in comparison to previous commits and red highlights means changes that were deleted.
```
...
```{r, prettyfig, fig.align='center', fig.cap="Difference between control and acidications treatments for each species."}
#plot1 = ggplot(data = sum_table, mapping = aes(x = species, y = mean_activity, color=treatment)) +
  #geom_point() + annotation_raster()
...
ggsave(path = "output/figures", filename = "fig1.png")
```
...
```{r}
## There is no changed file element to commit in relation to the ggplot figure that was saved.
```
...
```{r, answer6}
## No. Github isn't a system to backup files. Since the pictures won't change there is no need to track them.
```
...
```{r, collabFig, echo=TRUE, eval=TRUE, fig.cap= "Collaboration Figure"}
...
sum_table %>% filter(species == "chromis" | species == "lemon" | species == "acantho") %>% ggplot(sum_table, mapping = aes(x = species, y = mean_activity, color=treatment)) + geom_point()
...
n_reps <- unique(data["species"])

control = (data[which(data$species == 'ambon' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'ambon' & data$treatment == "CO2"),])
ambon = t.test(control$activity,CO2$activity)

control = (data[which(data$species == 'lemon' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'lemon' & data$treatment == "CO2"),])
lemon = t.test(control$activity,CO2$activity)

control = (data[which(data$species == 'chromis' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'chromis' & data$treatment == "CO2"),])
chromis = t.test(control$activity,CO2$activity)

control = (data[which(data$species == 'whitedams' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'whitedams' & data$treatment == "CO2"),])
whitedams = t.test(control$activity,CO2$activity)

control = (data[which(data$species == 'acantho' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'acantho' & data$treatment == "CO2"),])
acantho = t.test(control$activity,CO2$activity)

control = (data[which(data$species == 'humbug' & data$treatment == "control"),])
CO2 = (data[which(data$species == 'humbug' & data$treatment == "CO2"),])
humbug = t.test(control$activity,CO2$activity)


df = data_frame(
  species = c("ambon","lemon","chromis","whitedams","acantho","humbug"),
  diff_means = c(abs(ambon$estimate[[1]]-ambon$estimate[[2]]),abs(lemon$estimate[[1]]-lemon$estimate[[2]]),abs(chromis$estimate[[1]]-chromis$estimate[[2]]),abs(whitedams$estimate[[1]]-whitedams$estimate[[2]]),abs(acantho$estimate[[1]]-acantho$estimate[[2]]),abs(humbug$estimate[[1]]-humbug$estimate[[2]])),
  con_int = c(toString(round(ambon$conf.int,2)),toString(round(lemon$conf.int,2)),toString(round(chromis$conf.int,2)),toString(round(whitedams$conf.int,2)),toString(round(acantho$conf.int,2)),toString(round(humbug$conf.int,2))),
  t_stat = c(ambon$statistic,lemon$statistic,chromis$statistic,whitedams$statistic,acantho$statistic,humbug$statistic),
  df = c(ambon$parameter,lemon$parameter,chromis$parameter,whitedams$parameter,acantho$parameter,humbug$parameter),
  p_value = c(ambon$p.value,lemon$p.value,chromis$p.value,whitedams$p.value,acantho$p.value,humbug$p.value)
)
...
```{r, table1, echo=TRUE, eval=TRUE, tab.cap = "T-test statistics based on different treatments for each species"}
...
Mean activity for `r "ambon"` in the control group was `r "ambon$estimate[[1]]"` (s / min) compared to the OA treatment group, which was `r "ambon$estimate[[2]]"` (s / min). The difference between control and OA treatment means was `r "df$diff_means[[1]][1]"` (s / min) (95% CI: `r "ambon$conf.int[[1]]"` to `r "ambon$conf.int[[2]]"`)
